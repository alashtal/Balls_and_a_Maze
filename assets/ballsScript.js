...
function GenerateRandomColor() {
    ...
    var color = '#';
    for (var i = 0; i < 6; i++) {
        // For every i less 6 times assgin to colors + # + i
        color += letters[Math.floor(Math.random() * 16)];
        // Floor the the number (So result is not double) and produce a number betwen 0 and x16 Hex colors
    };
    return color;
}
...
function DrawTheBall() {
    ...
    this.radius = Math.random() * 20 + 13; /* ball random radius size should change everytime */
